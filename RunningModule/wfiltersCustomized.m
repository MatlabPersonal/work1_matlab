function [Lo_R,Hi_R] = wfiltersCustomized(wname)
%function varargout = wfiltersCustomized(wname,o)
...
if wtype==1                % orth. wavelet
    ...
    %else
        %F = feval(fname,wname);
    if(strcmp(wname,'db3'))
%       F=[0.3415    0.5915    0.1585   -0.0915];
        F=[0.2352 0.5706 0.3252 -0.0955 -0.0604 0.0249];
    elseif (strcmp(wname,'db5'))
        F=[0.1132 0.4270 0.5122 0.0979 -0.1713 -0.0228 0.0549 -0.0044 -0.0089 0.0024];
    elseif (strcmp(wname,'sym3'))
        F=[0.2352 0.5706 0.3252 -0.0955 -0.0604 0.0249];
    end
    %end
   % [Lo_D,Hi_D,Lo_R,Hi_R] = orthfiltCustomized(F);
    [~,~,Lo_R,Hi_R] = orthfiltCustomized(F);

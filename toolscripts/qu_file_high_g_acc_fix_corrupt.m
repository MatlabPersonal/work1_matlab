function [acc,gyro,mag,high_g_acc,battery_soc,battery_voltage,time,timeIndex,timeInterp,stc,page_info,raw_data] = qu_file_high_g_acc_fix_corrupt(filename)
%
...
A = m.Data;
raw_data = double(reshape(A,[2048,length(A)/2048])');
...
cut_data = raw_data;
% assignin('base','cut_data',cut_data);
...
%max value is +-2000
gyro = convert2signed_int(gyro)*500/32768; % Natalie is +-250deg/s
...
disp('-----------------------------------------------------------');
fprintf('decoding: %s\n',filename);
fprintf('estimated end time: %.2f secs\n',estimated_end_time/1000);
fprintf('\nsampling rate of TZ acc/gyro is %.2fHz\n',rate_low);
if(rate_mag == 0)
    mag_disable_flag = true;
    disp('mag disabled');
else
    mag_disable_flag = false;
    fprintf('sampling rate of TZ mag is %.2fHz\n',rate_mag);
end
if(rate == 0)
    highg_disable_flag = true;
    disp('high g acc disabled');
else
    highg_disable_flag = false;
    fprintf('sampling rate of high g is %.2fHz\n',rate);
end

% 26/05/2017 pad zeros for lost page and corrupted data
...
for i = 1:nbr_of_pages-1
    ...
    if(page_info(current_page,1) < page_sample_med_acc - 1)
        %corrupted_page
        cursor = sum(page_info(1:current_page,1));
        acc_interp_temp = [linspace(acc(cursor-1,1),acc(cursor+1,1),page_sample_med_acc - page_info(current_page,1))' ...
            linspace(acc(cursor-1,2),acc(cursor+1,2),page_sample_med_acc - page_info(current_page,1))' ...
            linspace(acc(cursor-1,3),acc(cursor+1,3),page_sample_med_acc - page_info(current_page,1))'];

        gyro_interp_temp = [linspace(gyro(cursor-1,1),gyro(cursor+1,1),page_sample_med_acc - page_info(current_page,2))' ...
            linspace(gyro(cursor-1,2),gyro(cursor+1,2),page_sample_med_acc - page_info(current_page,2))' ...
            linspace(gyro(cursor-1,3),gyro(cursor+1,3),page_sample_med_acc - page_info(current_page,2))'];
        
        acc = [acc(1:cursor,:);acc_interp_temp;acc(cursor+1:end,:)];
        gyro = [gyro(1:cursor,:);gyro_interp_temp;gyro(cursor+1:end,:)];
        page_info(current_page,1) = page_sample_med_acc;
        page_info(current_page,2) = page_sample_med_acc;
        num_corr = num_corr + 1;
    end
    if(page_info(current_page + 1,4) - page_info(current_page,4)> th)
        %lost page
        cursor = sum(page_info(1:current_page,1));
        acc_interp_temp = [linspace(acc(cursor-1,1),acc(cursor+1,1),page_sample_med_acc)'...
            linspace(acc(cursor-1,2),acc(cursor+1,2),page_sample_med_acc)'...
            linspace(acc(cursor-1,3),acc(cursor+1,3),page_sample_med_acc)'];
        gyro_interp_temp = [linspace(gyro(cursor-1,1),gyro(cursor+1,1),page_sample_med_acc)' ...
            linspace(gyro(cursor-1,2),gyro(cursor+1,2),page_sample_med_acc)' ...
            linspace(gyro(cursor-1,3),gyro(cursor+1,3),page_sample_med_acc)'];
        
        acc = [acc(1:cursor,:);acc_interp_temp;acc(cursor+1:end,:)];
        gyro = [gyro(1:cursor,:);gyro_interp_temp;gyro(cursor+1:end,:)];
        page_info(current_page,1) = page_info(current_page,1)+page_sample_med_acc;
        page_info(current_page,2) = page_info(current_page,2)+page_sample_med_acc;
        num_lost = num_lost + 1;
    end
...
timeStamp = [page_info(2:end,4); mean(diff(page_info(:,4)))*page_info(end,1)/mean(page_info(:,1))+page_info(end,4)]; %estimate the last timeStamp
accTime = zeros(1,sum(page_info(:,1)));
accTimeIndexTemp = cumsum(page_info(:,1));

% first page
accTime(1:accTimeIndexTemp(1)) = linspace(1,timeStamp(1),page_info(1,1)); %1:timeStamp(1)/page_info(1,1):timeStamp(1);
gyroTime = zeros(1,sum(page_info(:,2)));
gyroTimeIndexTemp = cumsum(page_info(:,2));
gyroTime(1:gyroTimeIndexTemp(1)) = linspace(1,timeStamp(1),page_info(1,2));%1:timeStamp(1)/page_info(1,2):timeStamp(1);


average_sample_time = 1000/rate_low;
% rest of the pages
for i = 2:nbr_of_pages
    accTime(accTimeIndexTemp(i-1)+1:accTimeIndexTemp(i)) = linspace(timeStamp(i-1)+average_sample_time,timeStamp(i),page_info(i,1)); %timeStamp(i-1)+average_sample_time:(timeStamp(i) - timeStamp(i-1)-average_sample_time)/page_info(i,1):timeStamp(i);
    gyroTime(gyroTimeIndexTemp(i-1)+1:gyroTimeIndexTemp(i)) = linspace(timeStamp(i-1)+average_sample_time,timeStamp(i),page_info(i,2)); %timeStamp(i-1)+average_sample_time:(timeStamp(i) - timeStamp(i-1))/page_info(i,2):timeStamp(i);
end
% use below if using timeStamps
[accTime] = forceMonotonicIncrease(accTime);
[gyroTime] = forceMonotonicIncrease(gyroTime);
...
if(~mag_disable_flag)
    ...
    for i = 2:nbr_of_pages
        magTime(magTimeIndexTemp(i-1)+1:magTimeIndexTemp(i)) = linspace(timeStamp(i-1)+average_sample_time,timeStamp(i),page_info(i,3));%timeStamp(i-1)+average_sample_time:(timeStamp(i) - timeStamp(i-1)-average_sample_time)/page_info(i,3):timeStamp(i);
    end
    mag = interp1(magTime,mag,timeInterp);
    mag(isnan(mag)) = 0;
end

if(~highg_disable_flag)
...
disp('-----------------------------------------------------------');
    ...
    function [out] = forceMonotonicIncrease(in)
        fprintf('\n**force timestamp monotonic increase, if stuck too long abort manully**\n');
        isfinished = false;
        while(~isfinished)
            ...
            if(isempty(ind1))
                ...
                fprintf('remainning bad timestamp: %d, first one at (%d)\n',length(ind1),ind1(1));
                disp(in(ind1(1)));
                disp(in(ind1(1)+1));
                disp(in(ind1(1)-1));
            end
        end
        fprintf('\n**force timestamp monotonic completed**\n');
        out = in;
    end
end

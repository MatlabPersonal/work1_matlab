...
for k=1:10000
    ...
    if(k<7)
        ...
        data{k}=zeros(1000,19);
        
        if(k>8000)
            data{k}(:,2:end)=0;
        elseif(k>6000)
            data{k}(:,2:size(data{k},2))=rand([size(data{k},1),size(data{k},2)-1])*rand()*200;
        elseif(k>4000)
            for nn=1:size(data{k},2)-1
                data{k}(:,nn+1)=sin(0.1*(1:size(data{k},1))'+rand()*2*pi)*rand()*200+rand()*100;
            end
        elseif(k>2000)
            data{k}=ones(1,19)*rand()*1000;
        end
    end
...
%     if(k==5)
%         accX1 = 1;accY1 = 1;accZ1 = 1;
%         accX2 = 1;accY2 = 1;accZ2 = 1;
%         gyroX1 = 1;gyroY1 = 1;gyroZ1 = 1;
%         gyroX2 = 1;gyroY2 = 1;gyroZ2 = 1;
%         stc = 1;
%     end
...
%         CalFx_low, CalLfx_low, CalFx_up, CalLfx_up);
    ...
    EulerS{k,1}=[EulerS1', EulerS2', EulerS3']/180*pi;
    if(~mod(k,100))

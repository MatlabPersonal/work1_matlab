function x = wextendCustomized(type,mode,x,lf,location)
...
%           y = cell(1,3);
%           for k=1:3
%               y{k} = wextendCustomized('2D',mode,x(:,:,k),lf,[locRow locCol]);
%           end
%           x = cat(3,y{:});
          return
      end
   ...
   case {'ar','ac','addrow','addcol'}
      ...
      if ismatrix(x)
          x = wextendCustomized('2D',mode,x,lf,location);
      else
          y = cell(1,3);
          x = double(x);
          for k=1:3
              y{k} = wextendCustomized('2D',mode,x(:,:,k),lf,location);
          end
          x = cat(3,y{:});
      end
end

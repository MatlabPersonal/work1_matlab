function [out1,out2,out3,out4,out5] = wavefunCustomized(wname,in2)
...
switch wtype
    case 1
      %[Lo_R,Hi_R] = wfiltersCustomized(wname,'r');
      [Lo_R,Hi_R] = wfiltersCustomized(wname);
      long  = length(Lo_R);
      nbpts = (long-1)/pas+1;
      phi   = coef*upcoefCustomized('a',1,wname,Lo_R,'dummy',iter);
      psi   = coef*upcoefCustomized('d',1,wname,Lo_R,Hi_R,iter);

      [nbpts,nb,dn] = getNBpts(nbpts,iter,long);
      phi = [0 wkeep1Customized(phi,nb) zeros(1,1+dn)];
      psi = [0 wkeep1Customized(psi,nb) zeros(1,1+dn)];
...
% 
%       [nbpts,nb,dn] = getNBpts(nbpts,iter,long);
%       phi1  = [0 wkeep1Customized(phi1,nb) zeros(1,1+dn)];
%       psi1  = [0 wkeep1Customized(psi1,nb) zeros(1,1+dn)];
% 
%       long  = length(Lo_R2);
%       % Hi_R2 = wrev(Hi_D2);
% 
...
% 
%       [nbpts,nb,dn] = getNBpts(nbpts,iter,long);
%       phi2 = [0 wkeep1Customized(phi2,nb) zeros(1,1+dn)];
%       psi2 = [0 wkeep1Customized(psi2,nb) zeros(1,1+dn)];
%       out1 = phi1; out2 = psi1;
%       out3 = phi2; out4 = psi2;

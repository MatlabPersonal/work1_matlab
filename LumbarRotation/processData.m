...
function [EulerS1, EulerS2, EulerS3,...
          ...
          EulerUpperS1, EulerUpperS2, EulerUpperS3] = processData(accX1,accY1,accZ1,accX2,accY2,accZ2,...
...
% Sensor 1
% get initial dip angle
acc_fx1=atan2(-acc1(:,2),acc1(:,3)+eps);
acc_lf1=asin(-acc1(:,1)./(mag_acc_low+eps));
% integrate gyro data into quaternions
q_vm1=quatint_stc(angle2quat(mean(acc_fx1(1:5)),mean(acc_lf1(1:5)),0,'xyz'),gyr1,stc/1000);
q_vm1_a=angle2quat(acc_fx1,acc_lf1,zeros(length(accX1),1),'xyz');

% similar process for Sensor 2
acc_fx2=atan2(-acc2(:,2),-acc2(:,3)+eps);
acc_lf2=asin(acc2(:,1)./(mag_acc_upp+eps));
q_vm2=quatint_stc(angle2quat(-mean(acc_fx2(1:5)),mean(acc_lf2(1:5)),0,'xyz'),gyr2,stc/1000);
q_vm2_a=angle2quat(-acc_fx2,acc_lf2,zeros(length(accX1),1),'xyz');
q_rot=angle2quat(pi,0,pi,'xyz'); %rotate upper around y axis by 180 degs
...
% get Euler angles for lower, upper, and diff and fix gyro drift with dip
% angles
EulerS=quat2XYZ(q_diff);
EulerAS=quat2XYZ(q_diff_a);
EulerS(:,1:2)=linear_fix(EulerS(:,1:2),EulerAS(:,1:2),5);
% EulerS(:,1)=-EulerS(:,1);
% EulerS(:,3)=-EulerS(:,3);
...
EulerS3=EulerS3-EulerS3(1);
% Euler Lower
EulerLower = quat2XYZ(q_vm1_c);
EulerLower_a = quat2XYZ(q_vm1_ac);
EulerLower(:,1:2)=linear_fix(EulerLower(:,1:2),EulerLower_a(:,1:2),5);
% Euler Upper
EulerUpper = quat2XYZ(q_vm2_c);
EulerUpper_a = quat2XYZ(q_vm2_ac);
EulerUpper(:,1:2)=linear_fix(EulerUpper(:,1:2),EulerUpper_a(:,1:2),5);

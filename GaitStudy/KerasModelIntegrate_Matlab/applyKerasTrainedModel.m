function [test_results] = applyKerasTrainedModel(test_data,net,norm_info)
%--------------------------------------------------------------------------
...
L2_weight = double(net{1,3}');
L2_bias = double(net{1,4}');
output_weight = double(net{1,5}');
output_bias = double(net{1,6});
test_results = sigmoid(output_weight*sigmoid(...
    L2_weight*sigmoid(...
    L1_weight*(test_data')...
    + L1_bias*ones(1,N))...
    + L2_bias*ones(1,N))+output_bias);
